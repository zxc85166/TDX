...
const getSpeedQuery = (type) => {
  axios({
    method: "get",
    url: `https://ptx.transportdata.tw/MOTC/v2/Tourism/ScenicSpot/?$select=Name%2CAddress%2CTicketInfo%2CPhone%2CLevel%2CPicture%2CDescription%2COpenTime%2CClass1&$filter=contains(Class1%2C'${type}')&$format=JSON`,
...
function getAttractions() {
  ...
  axios({
    method: "get",
    url: `https://ptx.transportdata.tw/MOTC/v2/Tourism/ScenicSpot/${city}?$select=Name%2CAddress%2CTicketInfo%2CPhone%2CLevel%2CPicture%2CDescription%2COpenTime%2CClass1&$filter=contains(Class1%2C'${type}')&$format=JSON`,
    headers: GetAuthorizationHeader(),
  })
    .then((res) => {
      const data = res.data;
